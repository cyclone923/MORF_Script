...
if __name__ == "__main__":
    rootPath = "/rhome/yangchen/shared/CleanMORF/randomOutput/MCTS/uniqueLinkers"
    for linker in os.listdir(rootPath):
        destDir = rootPath + "/" + linker + "/" + linker + "_deformation"
        os.chdir(destDir)
        f = open(linker + "-oldReward.txt", "w")
        f.writelines(str(calculateReward(destDir + "/" + linker + "-ave-force.d", 13, 200)))
        f.close()
