...
def findBestMatch(x, y):
    ...
    if minS==0 and maxS==5000:
        return 'False', maxS
    while maxS < 150 and threshold > 10:
        ...
        if minStemp == 'False':
            break
    if max(abs(derivation(inter.UnivariateSpline (x, y, s = maxS), np.array([e * (max(x) - min(x)) / 500 + min(x) for e in range(500 + 1)]), 0.01))) < threshold:
        return minS, maxS
    elif max(abs(derivation(inter.UnivariateSpline (x, y, s = minS), np.array([e * (max(x) - min(x)) / 500 + min(x) for e in range(500 + 1)]), 0.01))) < threshold:
        return minS, minS
    else:
        return 'False', 5000
