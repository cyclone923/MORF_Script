...
def findBestMatch(x, y):
    ...
    def findMatchRange(x, y, step, start, end):
        ...
        for trail in trailList:
            ...
            if max(abs(derivation(tempFunc, testPointsX, 0.01))) < threshold:
                if trail == 0:
    ...
    while maxS-minS > 5:
        ...
        minStemp, maxStemp = findMatchRange(x, y, delta, minS, maxS)
        if minStemp == 'False':
            break
        minS=minStemp
        maxS=maxStemp
    if minS==0 and maxS==5000:
        return 'False', maxS
    while maxS < 200 and threshold > 10:
        ...
        if minStemp == 'False':
            break
    if max(abs(derivation(inter.UnivariateSpline (x, y, s = maxS), np.array([e * (max(x) - min(x)) / 500 + min(x) for e in range(500 + 1)]), 0.01))) < 50:
        return minS, maxS
    elif max(abs(derivation(inter.UnivariateSpline (x, y, s = minS), np.array([e * (max(x) - min(x)) / 500 + min(x) for e in range(500 + 1)]), 0.01))) < 50:
        return minS, minS
    else:
        return 'False', 5000
...
def singleTask(lab):
    ...
    # res.close()
    # tranFeatures = transFeature(*features)
    moments = calcFileMoment(lab)
    bondMoments = bondMom(lab)
    return bondMoments, moments
    # return bondMoments, moments, features, tranFeatures
    # return features, tranFeatures
...
if __name__ == "__main__":
    ...
    for i in range(int(start),int(end)):
        ...
        lab = linkerDir
        if lab + "-bondMoment.npy" not in os.listdir(os.getcwd()):
            print(linkerDir)
            # bondMomentList, momentList, featuresList, transFeatureList = singleTask(lab)
            bondMomentList, atomMomentList = singleTask(lab)
            np.save(lab + "-bondMoment.npy", np.array(bondMomentList))
            np.save(lab + "-atomMoment.npy", np.array(atomMomentList))
    ...
    #                 lab = dir[:-12]
    #                 # bondMomentList, momentList, featuresList, transFeatureList = singleTask(lab)
    #                 featuresList, transFeatureList = singleTask(lab)
