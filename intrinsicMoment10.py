...
if __name__ == "__main__":
    ...
    #     if linkerDir + '-286Moments.txt' in os.listdir(os.getcwd()):
    #         os.remove(linkerDir + '-286Moments.txt')
    #     f = open(linkerDir + '-286NorMoments.txt', 'w')
        ...
        for d in range(1,31):
            ...
            for dir in os.listdir(depthDir):
                if dir[-11:] == "deformation":
                    ...
                    print(lab)
                    features = calcFileFeatures(lab + "-ave-force.d")
                    print(features)
                    exit(0)
                    # intriMoment(lab)
                    # momentList = singleTask(lab)
                    # f = open(lab + '-286NorMoments.txt', 'w')
                    # for i in range(len(momentList)):
                    #     f.writelines(str(momentList[i]))
                    #     if i != len(momentList) - 1:
                    #         f.writelines(",")
