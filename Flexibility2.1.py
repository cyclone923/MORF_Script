...
#        return "Error: PE min outside range"
    ...
    plt.legend()
    plt.xlabel('Strain (%)', family='Helvetica', fontsize=10, color='black')
    plt.ylabel('Force (Kcal/mol/A)', family='Helvetica', fontsize=10, color='black')
    plt.xticks(family='Helvetica', fontsize=10, color='black')
    plt.yticks(family='Helvetica', fontsize=10, color='black')
    plt.subplot(212)
    ...
    plt.legend()
    plt.xlabel('Strain (%)', family='Helvetica', fontsize=10, color='black')
    plt.ylabel('Potential Energy (Kcal/mol)', family='Helvetica', fontsize=10, color='black')
    plt.xticks(family='Helvetica', fontsize=10, color='black')
    plt.yticks(family='Helvetica', fontsize=10, color='black')
    plt.title('')
    ...
    for i in range(len(flexibilityC) - 1):
        if _countCF_ < aveThresholdC or abs(flexibilityC[i + 1][1] - (_aveCF_/_countCF_)) < 3.5 + fdNoiseC/1000.0:    #The Threshold was set randomly, in need of more trials.
            _aveCF_ += flexibilityC[i + 1][1]
            _countCF_ += 1
        if _countCB_ < aveThresholdC or abs(flexibilityC[-i - 2][1] - (_aveCB_/_countCB_)) < 3.5 + fdNoiseC/1000.0:    #The Threshold was set randomly, in need of more trials.
            _aveCB_ += flexibilityC[-i - 2][1]
            _countCB_ += 1
        if _countTF_ < aveThresholdT or abs(flexibilityT[i + 1][1] - (_aveTF_/_countTF_)) < 3.5 + fdNoiseT/1000.0:    #The Threshold was set randomly, in need of more trials.
            _aveTF_ += flexibilityT[i + 1][1]
            _countTF_ += 1
        if _countTB_ < aveThresholdT or abs(flexibilityT[-i - 2][1] - (_aveTB_/_countTB_)) < 3.5 + fdNoiseT/1000.0:    #The Threshold was set randomly, in need of more trials.
    ...
    plt.legend()
    plt.xlabel('Strain (%)', family='Helvetica', fontsize=10, color='black')
    plt.ylabel('Flexibility (Kcal/mol/A)', family='Helvetica', fontsize=10, color='black')
    plt.xticks(family='Helvetica', fontsize=10, color='black')
    plt.yticks(family='Helvetica', fontsize=10, color='black')
...
dirOrig = "/Users/weiyi/Documents/Projects/FD-curve/Flexibility-Test/"
os.chdir(dirOrig)
lab = "linker0"
print(singleEvaluationTask(lab))