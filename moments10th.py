...
def singleTask(lab):
    ...
    *The local directory should be in the deformation folder with the given lab
    ...
    _ifYesJumpC_, _ifYesJumpT_, _diffOfCT_, _diffOfTC_, _aveCB_, _aveCF_, _aveTB_, _aveTF_, fdNoiseC, fdNoiseT = calcFileFeatures(res)
    res.close()
    features = []
    #_aveCB_ = transAve(_aveCB_)
    features.append(_aveCB_)
    #_aveTB_ = transAve(_aveTB_)
    features.append(_aveTB_)
    #_aveCF_ = transAve(_aveCF_)
    features.append(_aveCF_)
    #_aveTF_ = transAve(_aveTF_)
    features.append(_aveTF_)
    #_ifYesJumpC_ = transJump(_ifYesJumpC_)
    features.append(_ifYesJumpC_)
    #_ifYesJumpT_ = transJump(_ifYesJumpT_)
    features.append(_ifYesJumpT_)
    #_diffOfCT_ = transDiff(_diffOfCT_)
    features.append(_diffOfCT_)
    #_diffOfTC_ = transDiff(_diffOfTC_)
    features.append(_diffOfTC_)
    #fdNoiseC = transNoise(fdNoiseC)
    features.append(fdNoiseC)
    #fdNoiseT = transNoise(fdNoiseT)
    features.append(fdNoiseT)
    '''
...
if __name__ == "__main__":
    ...
    rootPath = "/rhome/wzhan097/shared/CleanMORF/randomOutput/BFS/finalNode/depth5"
    ...
    for i in range(6395,110000):
        ...
        momentList = singleTask(linkerDir)
        f = open(linkerDir + '-0_10thMoments.txt', 'w')
        #f = open("/rhome/wzhan097/shared/CleanMORF/randomOutput/weiyi-test/"+linkerDir + '-0_10thMoments.txt', 'w')
...
#dirOrig = "/Users/weiyi/Documents/Projects/FD-curve/moments-Test"
'''
desFile = open('moments.tsv','w+')
desFile.write('features&moments Info\n')
desFile.close()
dirOrig = '/rhome/wzhan097/shared/Deform-Linkers/Deformation-Sweep-00-Uniaxial/'
dirOrig = '/Users/weiyi/Documents/Projects/FD-curve/'
for i in range(160000):
    if SetDirectory(i,dirOrig) == False:
        continue
    os.chdir(dirOrig+SetDirectory(i,dirOrig))
    result = singleTask("linker"+str(int(i)))
    if result == False:
        continue
    desFile = open(dirOrig+'moments.tsv','a')
    for e in result[0]:
        desFile.write(str(e)+'\t')
...
os.chdir(dirOrig + "/" + '15' + "/" + 'Depth12' + "/" + 'linker34_deformation/')
lab = "linker34"
result = singleTask(lab)
for e in result[0]:
    print(str(e)+'\t')
for e in result[1]:
    print(str(e)+'\t')
